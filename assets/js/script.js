...
var createQuery = function(queryInput, startYear, endYear) {
  var seriesOrMovie = document.querySelector("#movieOrSeries").value;
  if (seriesOrMovie === '2') {
    var seriesOrMovieQuery = '&type=movie';
  }
  else if (seriesOrMovie === '3') {
    var seriesOrMovieQuery = '&type=series';
  }
  else {
  ...
  if (queryInput) {
    var queryString = '&query=' + queryInput;
  }
  else {
    var queryString = '';
  }
  ...
  var netflixQueryUrl = "https://unogsng.p.rapidapi.com/search?country_andorunique=US&orderby=rating&audiosubtitle_andor=and&limit=100&subtitle=english&countrylist=78&audio=english&offset=0" + 
    queryString + 
    seriesOrMovieQuery + 
    startYearQuery + 
    endYearQuery;
